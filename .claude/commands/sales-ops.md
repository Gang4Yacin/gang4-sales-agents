...
#### 2ter-d — Archivage J+15 (silencieux — PAS de rappel Slack ici)

> ⚠️ **Les rappels de relances ne sont PLUS postés dans #sales-ops.** Par décision produit, le rappel hebdo des relances en attente vit dans le **brief Sales Strategist** (`/sales-strategist`, canal #sales-strategist), pas dans le run quotidien sales-ops. Ici, on se contente de la **gestion silencieuse du cycle de vie** (détection envoi/modif/expiration + archivage), sans poster de nudge.

Archivage uniquement :
- Pour les cards `en_attente_validation` avec `age_days = now() - proposed_at >= 15` :
  - `state='archived'`, `resolved_by='auto_archive'`, `archived_reason='no_action_after_15_days'`. Update Notion `État='Archived'`.
- N'ajoute **pas** de section "Relances en attente" au rapport sales-ops (elle est gérée par le strategist). Tu peux mentionner un archivage effectué dans les notes du run, mais sans CTA.
